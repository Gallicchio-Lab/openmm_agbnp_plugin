...
double ReferenceCalcAGBNPForceKernel::executeAGBNP2(ContextImpl& context, bool includeForces, bool includeEnergy) {
  //weights
  RealOpenMM w_evol = 1.0, w_egb = 0.0, w_vdw = 0.0;
  ...
  for(int i = 0; i < numParticles; i++){
	...
	//(d > rad1 + rad2) is a debug for now
	...
	if(verbose && volms > 0.002){
	  cout << "S " << " " << posms[0] << " " << posms[1] << " " << posms[2] << " " << volms << endl;
	}
      }
    }
  }

  //next step is to obtain the free volumes of the spheres collected above,
  // and then their self-volumes among themselves
